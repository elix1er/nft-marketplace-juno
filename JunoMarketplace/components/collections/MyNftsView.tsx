...
export default function MyNftsView({collections, stakedCollections}: {collections: any[], stakedCollections: any[]}) {
    ...
    useEffect(() => {
        ...
        getCosmWasmClient().then((client) => {
          console.log(stakedCollections)
            if(stakedCollections.length) stakedCollections.forEach(  (collection: any) => {
                if(collection.tokens.length > 0)
    ...
    return (
      <div className="py-24 bg-white sm:py-32">
        <div className="px-6 mx-auto max-w-7xl lg:px-8">
          ...
          >
            {JSON.stringify(collectionsResolved)}
            {collectionsResolved?.filter((f: any) => f.tokens.length > 0).map((col: any) => (
                <li key={col.collectionAddress}>
                { JSON.stringify(col) }
                { col.token_data?.imageUrl && ( <Image className="aspect-[3/2] w-full rounded-2xl object-cover"   width={600} height={400} src={col.token_data?.imageUrl } alt="" /> ) }
                <h3 className="mt-6 text-lg font-semibold leading-8 tracking-tight text-gray-900">{col.name}</h3>
                <p className="text-sm text-base leading-7 text-gray-600">{col.collectionAddress.substr(0,12) + '...'}</p>
                <ul role="list" className="flex mt-6 gap-x-6">
                  <li>
