...
//     "juno144sv7x7rqv9q3fvwn8hwhnsjq3g4eugnazf5m5kr6hjw52aj7auq7s7uyf";
...
//           client
//             .queryContractSmart(collection.collectionAddress, {
//               nft_info: { token_id: collection.tokens[0] },
//             })
//             .then(async (response) => {
...
//                   if (multihash) {
//                     collection.token_data = await fetchDataFromMultihash(
//                       multihash
//                     );
//                     setCollectionsResolved(collections);
//                     if (
//                       collection.token_data &&
//                       collection.token_data.contentType &&
//                       collection.token_data.contentType.startsWith(
//                         "application/json"
//                       )
//                     ) {
//                       if (collection.token_data.data.image) {
//                         const hash = parseMultihash(
//                           collection.token_data.data.image
//                         );
...
//             Found {collectionsResolved?.length} NFTs associated with your

//             are available to be listed
//           </p>
//         </div>
//         <ul
//           role="list"
...
//           {collectionsResolved.length && collectionsResolved
//             ?.filter((f: { tokens: string | any[] }) => f.tokens.length > 0)
//             .map((col: any) => (
//               <li key={col.collectionAddress}>
//                 {JSON.stringify(col)}
//                 <img
//                   className="aspect-[3/2] w-full rounded-2xl object-cover"
//                   src={col.token_data?.imageUrl}
//                   alt=""
//                 />
...
//                   {col.collectionAddress.substr(0, 12) + "..."}
//                 </p>
...
async function fetchDataFromMultihash(multihash: string) {
  const url = `https://gateway.pinata.cloud/ipfs/${multihash}`;
  try {
    const response = await fetch(url);

    if (!response.ok) {
      console.error(`Network response was not ok: ${response.statusText}`);
      // Retry with .json at the end of the URL
      const retryUrl = `${url}.json`;
      const retryResponse = await fetch(retryUrl);

      if (!retryResponse.ok) {
        console.error(`Network response was not ok: ${retryResponse.statusText}`);
        return null;
      }
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        return { contentType, data: null, imageUrl: retryUrl };
      }
    }
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if(data.image)
        return { contentType, imageUrl: data.image, data };
      else if(data.media)
        return { contentType, imageUrl: data.media, data };
      else 
        return { contentType, imageUrl: null, data };
    } else {
      return { contentType, imageUrl: url, data: null };
    }

  } catch (error) {
    console.error("Fetching data failed:", error);
    return { contentType: null, imageUrl: null };
  }
}
...
export default function MyNftsView() {
  ...
  const [collectionsResolved, setCollectionsResolved] = useState([] as any[]);
  // let nftMarketComposer = new JunoNftMarketplaceMsgComposer(address,marketplaceAddr);

  useEffect(() => {
    // let tempCollections = Object.assign({}, collections);
    // console.log({collections, collectionsStaked, balance, address})
    
    if (collections.data) {
      collections.data.forEach((collection: any) => {
        if (collection.tokens.length > 0) jMarketplaceClient?.client.queryContractSmart(collection.collectionAddress, {
              nft_info: { token_id: collection.tokens[0] },
            })
            .then(async (response) => {
              if (response && response.token_uri) {
                  ...
                  if (multihash) {
                    ...
                    if (
                      ...
                      if (collection.token_data.data.image) {
                        ...
                        if(hash)
                          collection.token_data.imageUrl = `https://gateway.pinata.cloud/ipfs/${hash}`;
                        else if(collection.token_data?.data?.image.startsWith("ipfs_link")) {
                          // WHY ?
                          const partialImage = collection.token_data?.data?.image;
                          console.log({partialImage})
                          
                          console.log({foundDumbImage:true, collection})

                          collection.token_data.imageUrl = collection.token_data?.data?.image.replace("ipfs_link/", "https://gateway.pinata.cloud/ipfs/" + multihash + "/" + partialImage.split("/")[1])
                          setCollectionsResolved(collections as any);
      ...
      <div className="px-6 mx-auto max-w-7xl lg:px-8">
        <div className="max-w-2xl mx-auto lg:mx-0">
          ...
          {/* {JSON.stringify(balance)} */}
            ...
            Found {collections.data?.length} NFTs associated with your
            wallet, while {" "} 
            {
              collections.data?.filter(
                (f: { tokens: string | any[] }) => f.tokens.length > 0
              ).length
            }{" "}
            are available to be listed
          </p>
        ...
        <ul
          ...
          className="grid max-w-2xl grid-cols-1 mx-auto mt-20 gap-x-8 gap-y-16 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3"
        >
          {collectionsResolved.data?.filter((f: any) => f.tokens.length > 0)
            .map((col: any) => (
              <li key={col.collectionAddress}>
                {/* {JSON.stringify(col)} */}
                {col.token_data?.imageUrl && (
                  <Image
                    className="aspect-[3/2] w-full rounded-2xl object-cover"
                    width={600}
                    height={600}
                ...
                <p className="text-sm text-base leading-7 text-gray-600">
                  {col.collectionAddress.substr(0, 12) + "..."}
                </p>
                <ul role="list" className="flex mt-6 gap-x-6">
                  <li>
